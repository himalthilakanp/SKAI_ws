...
NODE_IDS = [21, 22, 23, 24, 25, 26, 27]  # J1 to J6 + Node 27 Gripper

# Mechanical Scale Mapping
NODE_SCALING_FACTORS = {21: 1.0, 22: 1.0, 23: 1.0, 24: 0.4, 25: 1.0, 26: 1.0, 27: 1.0}
...
class RobotArmControlPanel(ctk.CTk):
    def __init__(self):
        ...
        self.absolute_positions = [0.5, 0.4, 0.6, 0.5, 0.5, 0.5, 0.015] 
        self.gripper_closed_state = True
        ...
        # Give background loop thread 200ms to accumulate incoming SDO frames before syncing
        self.after(200, self.synchronize_hardware_positions)
    ...
    def precompute_neon_leds(self):
        ...
        for i in range(16):
            pulse_strength = (math.sin(i * (math.pi / 8)) + 1) / 2 
    ...
    def setup_can_bus(self):
        """Initializes the CAN bus via python-can without deprecation parameters"""
        try:
            self.bus = can.interface.Bus(channel=CAN_BUS, interface=CAN_INTERFACE, bitrate=250000)
    ...
    def toggle_gripper(self):
        """Toggles the dynamic end-effector posture state between open and close profiles"""
        if self.gripper_closed_state:
            target = 0.019
            self.btn_gripper.configure(text="GRIPPER: OPENING", fg_color="#E67E22")
            self.gripper_closed_state = False
        else:
            target = 0.015
            self.btn_gripper.configure(text="GRIPPER: CLOSING", fg_color="#27AE60")
            self.gripper_closed_state = True

        self.absolute_positions[6] = target
        if self.bus:
            try:
                arbitration_id = (27 << 5) | MSG_ID_SET_INPUT_POS
                data = struct.pack('<fhh', target, 0, 0)
                msg = can.Message(arbitration_id=arbitration_id, data=data, is_extended_id=False)
                self.bus.send(msg)
                print(f"-> Gripper (Node 27) Target Written: {target}")
            except can.CanError as e:
                print(f"CAN Error writing packet profile to end effector: {e}")
    ...
    def build_ui(self):
        ...
        joint_names = ["J1 Base", "J2 Shoulder", "J3 Elbow", "J4 Wrist 1", "J5 Wrist 2", "J6 Wrist 3", "Gripper"]
        ...
        # 2. Simplified Status Text Counters
        self.heartbeat_labels = {}
        for i, node in enumerate(NODE_IDS):
            frame = ctk.CTkFrame(self.left_frame, fg_color="transparent")
            frame.pack(fill="x", padx=15, pady=2)

            display_name = f"J{i+1}" if i < 6 else "Gripper"
            lbl_count = ctk.CTkLabel(frame, text=f"{display_name} : 0", font=ctk.CTkFont(family="Consolas", size=22, weight="bold"))
        ...
        self.btn_gripper = ctk.CTkButton(preset_frame, text="TOGGLE GRIPPER", font=ctk.CTkFont(weight="bold", size=14),
                                         fg_color="#2980B9", hover_color="#1F618D", height=40,
                                         command=self.toggle_gripper)
        self.btn_gripper.pack(pady=(15, 5), padx=20, fill="x")
        ...
        for i, node in enumerate(NODE_IDS):
            ...
            # Row 1: Labels
            display_tag = f"J{i + 1}:" if i < 6 else "Gripper:"
            lbl_j = ctk.CTkLabel(input_container, text=display_tag)
            ...
            btn_single = ctk.CTkButton(input_container, text="Send", width=65, height=24,
                                       ...
                                       command=lambda idx=i: self.send_single_position(joint_index=idx))
        ...
        self.link_colors = ['#555555', '#3498DB', '#3498DB', '#ECf0F1', '#ECf0F1', '#E74C3C', '#F1C40F']  
        ...
        # DRIVE ERROR STATUS PANEL (MOVED BELOW ENCODER ESTIMATES)
    ...
    def poll_sdo_endpoints(self):
        ...
        if self.bus:
            for node_id in NODE_IDS:
                endpoint_id = 704 if node_id in [21, 22, 23, 27] else 644
                        ...
                        if cmd_id == MSG_ID_HEARTBEAT and node_id in self.heartbeat_counts:
                            ...
                            if len(msg.data) >= 8:
                                try:
                                    ...
                                    if opcode == 0:  
                                        if (node_id in [21, 22, 23, 27] and ep_id == 704) or \
    ...
    def synchronize_hardware_positions(self):
        """Active Hardware Initialization Sync Engine
        Fires automatically 200ms after script boot. Samples non-volatile absolute hardware registers 
        and scales J4 tracking history memories prior to user input execution, removing the initial 0.25 jump.
        """
        for i, node_id in enumerate(NODE_IDS):
            enc_data = self.encoder_data[node_id]
            raw_data = self.raw_encoder_data[node_id]
            if raw_data is not None and enc_data is not None:
                turns = round(enc_data[0] - raw_data[0])
                current_abs_hw_pos = turns + raw_data[0]
                
                # ---> SYSTEM PATCH: Adjust initial software baseline values dynamically based on node reduction scales
                if node_id == 24:
                    self.absolute_positions[i] = current_abs_hw_pos / NODE_SCALING_FACTORS[24]
                else:
                    self.absolute_positions[i] = current_abs_hw_pos

        self.is_first_move = False
        print("Hardware Encoder Targets Aligned Successfully: ", self.absolute_positions)
    ...
    def update_gui_status(self):
        ...
        for i, node in enumerate(NODE_IDS):
            ...
            display_name = f"J{i+1}" if i < 6 else "Gripper"
            if count > 0:
                self.heartbeat_labels[node].configure(text=f"{display_name} : {count}", text_color="#00FF00")
            else:
                self.heartbeat_labels[node].configure(text=f"{display_name} : {count}", text_color="#E74C3C")
        ...
        if len(active_error_nodes) == 0:
            ...
            final_err_node_text = f"NODE {err_node}" if err_node != 27 else "GRIPPER"
            self.lbl_error_out.configure(text=f"{final_err_node_text} ERROR:\n{err_str}", text_color="#E74C3C")
            ...
            if self.selected_error_node in active_error_nodes:
                ...
                err_str = self.get_error_string(err_code)
                self.error_panel.configure(fg_color="#331414", border_color="#E74C3C")
                final_err_node_text = f"NODE {self.selected_error_node}" if self.selected_error_node != 27 else "GRIPPER"
                self.lbl_error_out.configure(text=f"{final_err_node_text} ERROR:\n{err_str}", text_color="#E74C3C")
            else:
    ...
    def send_single_position(self, joint_index):
        try:
            ...
            print(f"Warning: Invalid value at Joint index: {joint_index + 1}")
    ...
    def move_preset_home(self):
        ...
        home_offsets = [0.959, 0.548, 0.655, 0.160, 0.614, 0.000, 0.015]
        current_time = time.time()
        deltas = [0.0] * 7
    ...
    def move_preset_ext(self):
        ...
        ext_offsets = [0.959, 0.764, 0.060, 0.117, 0.614, 0.000, 0.015]
        current_time = time.time()
        deltas = [0.0] * 7
    ...
    def send_incremental_array(self, deltas):
        ...
        for i, node_id in enumerate(NODE_IDS):
            ...
            scale_multiplier = NODE_SCALING_FACTORS[node_id]
            
            # Rely on absolute stable software target memory tracker arrays for joints 3, 4, and 5
            if node_id in [23, 24, 25]:
                new_pos_accumulated = self.absolute_positions[i] + delta
                new_pos = new_pos_accumulated * scale_multiplier
                self.absolute_positions[i] = new_pos_accumulated
            else:
                new_pos = current_pos + (delta * scale_multiplier)
            ...
            if self.bus:
                try:
                    ...
                    self.bus.send(msg)
                    print(f"-> Node {node_id} Command Sent Step {delta:+.3f} -> Abs Target: {new_pos:.3f}")
                except can.CanError as e:
                    print(f"CAN Send Error on Node {node_id}: {e}")
            else:
                print(f"[MOCK MODE] Node {node_id} step {delta:+.3f} -> Target: {new_pos:.3f}")
    ...
    def calculate_fk(self, positions):
        angles = [p * 2 * np.pi for p in positions[:6]]  
    ...
    def update_arm_plot(self):
        ...
        self.canvas.draw_toggle() if hasattr(self.canvas, 'draw_toggle') else self.canvas.draw_idle()
